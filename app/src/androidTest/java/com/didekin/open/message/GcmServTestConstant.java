package com.didekin.open.message;

/**
 * User: pedro@didekin
 * Date: 11/01/17
 * Time: 15:20
 */

final class GcmServTestConstant {

    static final String PACKAGE_TEST = "com.didekin.open.message";

    // Api key of didekin project.
    private static final String API_KEY_TEST =
            "AAAADknoTJQ:APA91bGRihWJup9TYYtKl6LV7d01f5DZJDr5edlwh8KV4fLzq8S20OYyxnqP7Hsj2b4B4zDU0G_jzDH8bOwXGlz77XMFzcPWnEZ8EcDqTbiNTSjDHxuegT2eE8Dsn9YvozF4GIbIHFaJ";
    static final String test_api_key_header = "key=" + API_KEY_TEST;
    // This is the project number in google-services.json.
    static final String SENDER_ID = "61369502868";
    // Scope used to get a token.
    static final String SCOPE_TOKEN = "FCM";
}


